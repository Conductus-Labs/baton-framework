...
- **If `.baton/project.config.yml` doesn't exist:**
  ...
  - OR create from template: `src/core/templates/project-file-templates/project.config-template.yml`
  
- **If `.baton/project.manifest.md` doesn't exist:**
  - Create from template: `src/core/templates/project-file-templates/project.manifest-template.md`
  
- **If `.baton/boundaries/project-boundaries.md` doesn't exist:**
  - Create from template: `src/core/templates/boundaries-templates/project-boundaries-template.md`
...
1. **Run CLI command to list available agents:**
   ...
   - If CLI is not available, read from `src/core/agents/` directory
...
2. **Analyze project to suggest relevant agents:**
   ...
   **CRITICAL: Perform comprehensive analysis using multiple data sources:**
   ...
   f. **Read agent definitions if needed:**
      - If unsure about an agent's purpose, read the agent definition file from `src/core/agents/<agent-name>.md`
...
3. **Display suggestions to user:**
   ...
   - Replace placeholders:
     ...
     - `{all_agents_list}` - Formatted list of ALL available agents from `src/core/agents/` (include ALL agents, mark which are already installed)
...
6. **If user wants to select individually:**
...
8. **If user selected agents (all or individually):**
   - For each selected agent, run: `baton agent add <agent-name>`
   - OR manually copy from `src/core/agents/<agent-name>.md` to `.baton/agents/<agent-name>.md`
   - Update `project.config.yml` agents.enabled array with installed agents

9. **If user selected `skip`:**
...
1. **Run CLI command to list available workflows:**
   ...
   - If CLI is not available, read from `src/core/workflows/` directory (excluding sub-flows)
...
8. **If user selected workflows (all or individually):**
   - For each selected workflow, run: `baton workflow add <workflow-name>`
   - OR manually copy from `src/core/workflows/<workflow-name>.yml` to `.baton/workflows/<workflow-name>.yml`
...
1. **Run CLI command to list available knowledge files:**
   ...
   - If CLI is not available, read from `src/core/knowledge/` directory
...
8. **If user selected knowledge files (all or individually):**
...
1. **In baton-framework repository** (if this is the framework repo):
   - `src/core/commands/markdown/baton-agent/start-baton-project.md`
   - `src/core/commands/toml/baton-agent/start-baton-project.toml`
...
- If CLI is not available, read directly from `src/core/` directories
...
- **If CLI commands fail:**
  ...
  - Fall back to reading from `src/core/` directories
