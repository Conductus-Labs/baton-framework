...
export async function collectProjectConfig(npmPackageRoot: string, packageJson: any): Promise<any> {
  ...
  try {
    // Get remote URL
    gitRemoteUrl = execSync('git config --get remote.origin.url', { cwd: projectRoot, encoding: 'utf-8' }).trim();
  } catch {
  ...
  const prefAnswers = await inquirer.prompt([
    {
      ...
      message: 'Automatically save context after each session?',
      default: true
    },
    {
      type: 'confirm',
    ...
    },
    {
      ...
      message: 'Check for Baton Framework updates?',
      default: true
    },
    {
      type: 'confirm',
      name: 'send_feedback',
      message: 'Send usage feedback to improve Baton Framework?',
      default: false
    }
  ]);
...
export async function generateProjectConfig(config: any, npmPackageRoot: string, batonDir: string): Promise<void> {
  // Load template
  let templatePath = join(npmPackageRoot, 'src', 'core', 'templates', 'project-file-templates', 'project.config-template.yml');
  if (!existsSync(templatePath)) {
    // Fallback for development
    templatePath = join(npmPackageRoot, '..', 'core', 'templates', 'project-file-templates', 'project.config-template.yml');
  }
  
  if (!existsSync(templatePath)) {
    console.error(`\n❌ Error: project.config-template.yml not found\n`);
    process.exit(1);
  }
