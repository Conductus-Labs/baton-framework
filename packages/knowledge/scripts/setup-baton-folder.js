...
/**
 * Post-install script for @conductus-labs/baton-knowledge
...
function setupBatonFolder() {
  try {
    ...
    const batonKnowledgeDir = join(batonDir, "knowledge");

    // Create baton/knowledge directory if it doesn't exist
    if (!existsSync(batonKnowledgeDir)) {
      mkdirSync(batonKnowledgeDir, { recursive: true });
    }

    // Copy knowledge files (preserve directory structure)
    const srcKnowledge = join(packageDir, "src", "knowledge");
    if (existsSync(srcKnowledge)) {
      copyDir(srcKnowledge, batonKnowledgeDir);
    }

    console.log(
      "✓ baton-knowledge: Knowledge files copied to baton/knowledge/"
    );
  } catch (error) {
    console.warn("Error setting up baton-knowledge folder:", error.message);
